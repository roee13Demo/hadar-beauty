...
export function MobileNav() {
  ...
  return (
    <nav className="fixed inset-x-0 bottom-0 z-40 flex items-stretch border-t border-border/60 bg-card pb-[env(safe-area-inset-bottom)] md:hidden">
      {NAV_ITEMS.map((item) => {
        const isActive =
        ...
        return (
          <Link
            ...
            href={item.href}
            className={cn(
              "flex flex-1 flex-col items-center justify-center gap-1.5 py-4 text-[11px] font-medium transition-colors",
              isActive
                ? "text-primary"
